<template>
  <div class="category-container-wrapper">
    <div class="category-container">
      ...
      >
        ...
        <el-form-item class="btn-form-item">
          ...
          <el-button icon="el-icon-circle-plus-outline" @click="addCategory"
            >新增</el-button
          >
        </el-form-item>
      </el-form>
      ...
      >
        ...
        <el-table-column label="操作" align="center" min-width="150">
          <template v-slot="{ row }">
            ...
            <el-button type="danger" size="mini" @click="handlerDel(row.id)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    <category-edit
      ...
      :isAddCategory.sync="isAddCategory"
      :editFormData="editFormData"
      @refreshList="getCategoryListData"
    ></category-edit>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getCategorySearch() {
      ...
      this.currentPage = 1;
      this.pageSize = 10;
      const { data } = await getCategorySearch(odds);
      this.total = data.total;
      this.listData = data.records;
    },
    // 删除指定分类标签
    ...
    onSubmit: throttling(
      function () {
        if (!this.searchFormData.name && this.searchFormData.status === null) {
          this.$message({
...
.category-container-wrapper {
  ...
  .category-container {
    ...
    .form-search {
      @include flexLayout($jc: flex-start, $ai: center);
      padding-left: 20px;
